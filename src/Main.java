...
public class Main {
    ...
    static void editarPessoa(Scanner entrada, ArrayList<Pessoa> pessoas) {
        if (pessoas.isEmpty()) {
            System.out.println("Nenhuma pessoa cadastrada.");
            return;
        }

        System.out.println("Quem você quer editar? (pelo número)");
        ...
        System.out.print("Nova idade (maior que 0): ");
        int novaIdade = entrada.nextInt();
        entrada.nextLine(); // limpa Enter

        try {
            Pessoa nova = new Pessoa(novoNome, novaIdade);
            pessoas.set(indice, nova);
